...
export const generateForensicCertificate = async (result: AnalysisResult): Promise<string> => {
  const { response } = await safeInvoke("gemini-3-flash-preview", `Generate a detailed forensic certificate for Case ID ${result.id}.
...
export const reverseSignalGrounding = async (file: File): Promise<any> => {
  ...
  const { response, isSafeMode } = await safeInvoke("gemini-3-flash-preview", {
    parts: [
...
export const analyzeMedia = async (file: File, metadata: any): Promise<AnalysisResult> => {
  ...
  const { response, isSafeMode } = await safeInvoke("gemini-3-pro-preview", {
    parts: [
      { inlineData: { mimeType: file.type, data: base64Data } },
      { text: prompt }
    ]
...
export const analyzeText = async (text: string, mode: 'AI_DETECT' | 'FACT_CHECK'): Promise<TextAnalysisResult> => {
  ...
  const { response, isSafeMode } = await safeInvoke('gemini-3-flash-preview', text, {
    responseMimeType: "application/json",
...
export const transcribeAudio = async (audioBlob: Blob): Promise<string> => {
  ...
  const { response } = await safeInvoke("gemini-3-flash-preview", {
    parts: [
