...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ result, onReupload, onOpenReport, onBack }) => {
  ...
  const handleExport = () => {
    ...
    downloadAnchorNode.setAttribute("download", `TRUSTY_CASE_${result.id}.json`);
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pt-12 pb-24 selection:bg-neon selection:text-charcoal relative">
      ...
      <div className="flex flex-wrap items-center justify-between gap-6 mb-10 px-4">
        ...
        <div className="flex gap-4">
          ...
          <button 
            onClick={onReupload} 
            className="px-5 py-2.5 rounded-xl bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-neon transition-all shadow-xl shadow-white/10 hover:shadow-neon/20 flex items-center gap-2"
          >
            <Home size={14} /> New Scan
          </button>
        </div>
      </div>
